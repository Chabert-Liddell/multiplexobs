...
class MultiPlexObs(nn.Module):
    def __init__(self, 
            ...
            Args:
                ...
                nb_blocks_net (int): The number of blocks of the true network.
            ...
            self.nb_blocks_net = torch.tensor(nb_blocks_net) 
            ...
            assert len(nb_blocks_obs) == nb_clusters, "The length of nb_blocks_obs must be the same as nb_clusters!"
            assert np.all(nb_blocks_obs == nb_blocks_obs[0]), "Each observation process must have the same number of blocks!"
            ...
            if self.obs_dist == 'Beta':
                self.beta_ss = torch.randn(1, dtype=torch.float32).to(self.device).requires_grad_(True)
            self.tau_lat = torch.randn(nb_nodes, nb_blocks_net - 1, dtype=torch.float32).to(self.device).requires_grad_(True)
            self.tau_obs = [torch.randn(nb_nodes, q - 1, dtype=torch.float32).to(self.device).requires_grad_(True) for q in nb_blocks_obs]
            self.tau_net = torch.randn(nb_networks, nb_clusters - 1, dtype=torch.float32).to(self.device).requires_grad_(True)
            self.alpha_obs_pos = [(1 + torch.randn(q, q, dtype=torch.float32)).to(self.device).requires_grad_(True) for q in nb_blocks_obs]
            self.alpha_obs_neg = [(- 1 + torch.randn(q, q, dtype=torch.float32)).to(self.device).requires_grad_(True) for q in nb_blocks_obs]
            self.alpha_lat = torch.randn(nb_blocks_net, nb_blocks_net, dtype=torch.float32).to(self.device).requires_grad_(True)
            self.pi_lat = torch.randn(nb_blocks_net - 1, dtype=torch.float32).to(self.device).requires_grad_(True)
            
            if self.is_hierarchical:
                self.pi_obs = [torch.randn(nb_blocks_net, q - 1, dtype=torch.float32).to(self.device).requires_grad_(True) for q in nb_blocks_obs]
            else:
                self.pi_obs = [torch.randn(1, q - 1, dtype=torch.float32).to(self.device).requires_grad_(True) for q in nb_blocks_obs]            
                
            if self.is_dynamic:
                self.pi_net = torch.randn(nb_clusters, nb_clusters - 1, dtype=torch.float32).to(self.device).requires_grad_(True)
            else:
                self.pi_net = torch.randn(nb_clusters - 1, dtype=torch.float32).to(self.device).requires_grad_(True)

            if net_covariates is not None:
                self.pi_net = torch.randn(self.nb_covariates, nb_clusters - 1, dtype=torch.float32).to(self.device).requires_grad_(True)
            else:
                self.pi_net = torch.randn(nb_clusters - 1, dtype=torch.float32).to(self.device).requires_grad_(True)            

            self.params = [self.tau_lat, *self.tau_obs, self.tau_net, *self.alpha_obs_pos, *self.alpha_obs_neg, self.alpha_lat,
                            self.pi_lat, *self.pi_obs, self.pi_net]
            if self.obs_dist == 'Beta':
    ...
    def initialize(self, data):
        ...
        self.present =  1.*(data.M.sum(dim=1) + data.M.sum(dim=2) > 0) 
        self.p_present = [self.present.mean() + torch.zeros(q, dtype = torch.float32).to(self.device) for q in self.nb_blocks_obs]
    ...
    def initialize_variational(self, tau_obs=None, tau_net=None, tau_lat=None, init_type='random'):
            ...
            with torch.no_grad():
                ...
                if tau_obs is not None:
                    tau_obs = [pcf.simplex_to_reals(tau_obs[k]) for k in range(self.nb_clusters)]
                    if init_type == 'random':                
                        ...
                        [self.tau_obs[k].mul_(0).add_(tau_obs[k]) for k in range(self.nb_clusters)]
    ...
    def natural_parameters_obs(self):
            ...
            alpha_obs_neg = [torch.sigmoid(self.alpha_obs_neg[k]) for k in range(self.nb_clusters)]
            if self.directed == False:
                for k in range(self.nb_clusters):
                    alpha_obs_pos[k] = alpha_obs_pos[k].tril() + alpha_obs_pos[k].tril(diagonal = -1).t()
                    alpha_obs_neg[k] = alpha_obs_neg[k].tril() + alpha_obs_neg[k].tril(diagonal = -1).t()
    ...
    def natural_parameters_entropy(self):
            ...
            tau_obs = [pcf.reals_to_simplex(self.tau_obs[k]) for k in range(self.nb_clusters)]
            return tau_obs, tau_net, tau_lat
    
    
    def train(self, dataloader, optimizer, nb_epochs = 500, loss = 'elbo', 
        ...
        for epoch in range(self.epochs):
            ...
            for net, mask, id, labels in dataloader:
                ...
                if self.loss == 'elbo':
                    ...
                    complete_log_likelihood = self.complete_log_likelihood(net, mask, id, labels)
                ...
                optimizer.step()
                with torch.no_grad():
                    self.alpha_lat.clamp_(-10,10)
                    for k in range(self.nb_clusters):
                        self.pi_obs[k].clamp_(-10, 10)                         
                        self.tau_obs[k].clamp_(-10,10)
                        self.alpha_obs_pos[k].clamp_(-10,10)
                        self.alpha_obs_neg[k].clamp_(-10,10)
                    self.pi_lat.clamp_(-10, 10)
                    self.tau_lat.clamp_(-10,10)           
                    self.tau_net.clamp_(-10,10)           
                    ...
                    self.A_lat = (1-data_ratio)*self.A_lat + \
                        data_ratio*self.ve_step_A(net, mask, tau_net[id,:], tau_obs, tau_lat, 
                                                  alpha_obs_pos, alpha_obs_neg, alpha_lat)                      
                    for k in range(self.nb_clusters):
                        self.p_present[k] = (1 - data_ratio)*self.p_present[k] + \
                            data_ratio *(tau_net[id,k] @ self.present[id,:] @ tau_obs[k]) / \
            ...
            if verbose & (epoch % 10 == 9):            
                print('[%d] loss: %.2f, loglik: %.2f, entropy: %.2f'%
                (epoch + 1, 
    ...
    def complete_log_likelihood(self, A, M, net_id, net_labels):
        ...
        cll = 0
        # obs likelihood
        cll += self.obs_likelihood(A, M, lat_net, self.M_lat, 
                                   alpha_obs_pos, alpha_obs_neg, tau_obs, tau_net[net_id,:])
        # Latent network likelihood
        cll += (nb_net/self.nb_networks) * \
            self.net_likelihood(lat_net, self.M_lat, alpha_lat, tau_lat)
        # Latent network mixture likelihood
        cll += (nb_net/self.nb_networks) * torch.sum(tau_lat @ pi_lat.log())        
        # Observation block likelihood
        for k in range(self.nb_clusters):
            if self.is_hierarchical:
                cll += (nb_net/self.nb_networks) * torch.sum((tau_lat @ pi_obs[k].log()) * tau_obs[k])
            else:
                cll += (nb_net/self.nb_networks) * torch.sum(tau_obs[k] @ pi_obs[k][0,:].log())                         
        if self.is_dynamic:
            cll += (nb_net/self.nb_networks)*(torch.sum((tau_net[0:-1,].T @ tau_net[1:,]) * pi_net.log()))
            if self.net_covariates is not None:
                cll += (nb_net/self.nb_networks)*torch.sum(
                    torch.einsum('ij,ij->i', tau_net, pi_net.log()[self.net_covariates,:]))
            else:
                cll += torch.sum(tau_net[net_id,:] @ pi_net.log())

        # Value for nodes class sampling
        for k in range(self.nb_clusters):
            cll +=  (tau_net[net_id,k] @ self.present[net_id,:] @ tau_obs[k] @ self.p_present[k].log())
            cll += (tau_net[net_id,k] @ (1-self.present[net_id,:]) @ tau_obs[k] @ (1-self.p_present[k]).log())
        return cll            
    ...
    def entropy(self, net_id, net_labels):
        ...
        entropy = 0
        for k in range(self.nb_clusters):
            entropy -= (nb_net/self.nb_networks) * torch.sum(torch.xlogy(tau_obs[k], tau_obs[k]))
        if self.is_dynamic:
            entropy -= (nb_net/self.nb_networks)*torch.sum(torch.xlogy(tau_net, tau_net))
        else:
            entropy -= torch.sum(torch.xlogy(tau_net[net_id,:], tau_net[net_id,:]))
        entropy -= (nb_net/self.nb_networks) * torch.sum(torch.xlogy(tau_lat , tau_lat))
        entropy -= (nb_net/self.nb_networks) * torch.sum(torch.xlogy(self.M_lat*lat_net , lat_net)) 
        entropy -= (nb_net/self.nb_networks) * torch.sum(torch.xlogy(self.M_lat*(1-lat_net) , 1-lat_net)) 
        return entropy
    ...
    def obs_likelihood(self, A_obs, M_obs, A_lat, M_lat, alpha_pos, alpha_neg, tau_nodes, tau_net):
        
        if self.obs_dist == 'Bernoulli':
            ll = F.bernoulli_obs_likelihood(A_obs, M_obs, A_lat, M_lat, alpha_pos, alpha_neg, tau_nodes, tau_net)
        if self.obs_dist == 'ContinuousBernoulli':
            ll = F.continuous_bernoulli_obs_likelihood(A_obs, M_obs, A_lat, M_lat, alpha_pos, alpha_neg, tau_nodes, tau_net)
        if self.obs_dist == 'Beta':
            ll = F.beta_obs_likelihood(A_obs, M_obs, A_lat, M_lat, alpha_pos, alpha_neg, tau_nodes, tau_net, self.beta_ss)            

        return ll
    

    def ve_step_A(self, net, mask, q_net, q_obs, q_lat, alpha_pos, alpha_neg, alpha_lat):
        """
        ...
        """
        A_prior = q_lat @ alpha_lat.logit() @ q_lat.T     
        f_obs_pos = 0
        f_obs_neg = 0
        L = q_net.shape[0]
        if self.obs_dist in ('Bernoulli', 'ContinuousBernoulli'):
            for k in range(self.nb_clusters):
                ...
                f_obs_pos += X_pos*(q_obs[k] @ alpha_pos[k].log() @ q_obs[k].T)
                f_obs_pos += X_neg*(q_obs[k] @ (1-alpha_pos[k]).log() @ q_obs[k].T)
                f_obs_neg += X_pos*(q_obs[k] @ alpha_neg[k].log() @ q_obs[k].T)
                f_obs_neg += X_neg*(q_obs[k] @ (1-alpha_neg[k]).log() @ q_obs[k].T)                
                if self.obs_dist == 'ContinuousBernoulli':
                    X = torch.sum(q_net[:,k].view(L,1,1)*mask, axis=0)
                    f_obs_pos += X*(q_obs[k] @ F.log_norm_const(alpha_pos[k]) @ q_obs[k].T)
                    f_obs_neg += X*(q_obs[k] @ F.log_norm_const(alpha_neg[k]) @ q_obs[k].T)                    
        Pos = -1 + A_prior + f_obs_pos - f_obs_neg
        Neg = -1 - A_prior - f_obs_pos + f_obs_neg
        Max = torch.fmax(Pos,Neg)
        
        q_A = (Pos - Max).exp() / \
            ((Pos - Max).exp() + (Neg - Max).exp())     
        return (q_A)              
    ...
    def compute_penalty(self):
        ...
        pen_obs_mis = 0
        for k in range(self.nb_clusters):
            pen_obs_con += .5 * F.pen_inter(self.nb_blocks_obs[k], self.directed) * self.nb_inter_obs.log()
            if self.is_hierarchical:
                pen_obs_mix += .5 * self.nb_blocks_net*(self.nb_blocks_obs[k]-1) * self.nb_nodes_obs.log()
            else:
                pen_obs_mix += .5 * (self.nb_blocks_obs[k]-1) * self.nb_nodes_obs.log()
            pen_obs_mis += .5 * self.nb_blocks_obs[k] * self.nb_nodes_obs.log()
        pen_lat_con = .5 * F.pen_inter(self.nb_blocks_net, self.directed) * self.nb_inter_lat.log()
        pen_lat_mix = .5 * (self.nb_blocks_net - 1) * self.nb_nodes.log()
        pen_net_mix = 0
        if self.is_dynamic:
            pen_net_mix += .5*self.nb_clusters*(self.nb_clusters - 1)*(self.nb_networks-1).log()
        else:
            if self.net_covariates is None:
                pen_net_mix += .5*(self.nb_clusters - 1)*self.nb_networks.log()
            else:
                for c in range(self.nb_covariates):
                    pen_net_mix += .5*(self.nb_clusters - 1)*np.log(np.sum(self.net_covariates == c))                    
